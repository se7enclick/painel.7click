...
class Contracts_model extends Crud_model
{
    ...
    function get_details($options = array())
    {
        ...
        if ($deadline) {
            ...
            if ($deadline === "expired") {
                $where .= " AND ($contracts_table.valid_until IS NOT NULL AND $contracts_table.valid_until<'$now')";
            } else {
                $where .= " AND ($contracts_table.valid_until IS NOT NULL AND $contracts_table.valid_until<='$deadline')";
            }
        }
        ...
        $contracts_label = get_array_value($options, "contracts_label");
        if ($contracts_label) {
            $where .= " AND (FIND_IN_SET('$contracts_label', $contracts_table.labels)) ";
        }
        ...
        $sql = "SELECT $contracts_table.*, $clients_table.currency, $clients_table.currency_symbol, $clients_table.company_name, $clients_table.is_lead, $projects_table.title AS project_title, $select_labels_data_query,
    ...
    function get_contract_total_summary($contract_id = 0)
    {
        ...
        */
        ...
        $single_sql = "SELECT SUM($contract_items_table.total) AS sum_total_single
        FROM $contract_items_table
        LEFT JOIN $contracts_table ON $contracts_table.id= $contract_items_table.contract_id    
        WHERE $contract_items_table.recurrence='Único'
        AND $contract_items_table.deleted=0 
        AND $contract_items_table.contract_id=$contract_id 
        AND $contracts_table.deleted=0";

        $total_single = $this->db->query($single_sql)->getRow();
        ...
        */
        ...
        $monthly_sql = "SELECT SUM($contract_items_table.total) AS sum_total_monthly
        FROM $contract_items_table
        LEFT JOIN $contracts_table ON $contracts_table.id= $contract_items_table.contract_id    
        WHERE $contract_items_table.recurrence='Mensal'
        AND $contract_items_table.deleted=0 
        AND $contract_items_table.contract_id=$contract_id 
        AND $contracts_table.deleted=0";

        $total_monthly = $this->db->query($monthly_sql)->getRow();
        ...
        */
        ...
        $quarterly_sql = "SELECT SUM($contract_items_table.total) AS sum_total_quarterly
        FROM $contract_items_table
        LEFT JOIN $contracts_table ON $contracts_table.id= $contract_items_table.contract_id    
        WHERE $contract_items_table.recurrence='Trimestral'
        AND $contract_items_table.deleted=0 
        AND $contract_items_table.contract_id=$contract_id 
        AND $contracts_table.deleted=0";

        $total_quarterly = $this->db->query($quarterly_sql)->getRow();
        ...
        */
        ...
        $yearly_sql = "SELECT SUM($contract_items_table.total) AS sum_total_yearly
        FROM $contract_items_table
        LEFT JOIN $contracts_table ON $contracts_table.id= $contract_items_table.contract_id    
        WHERE $contract_items_table.recurrence='Anual'
        AND $contract_items_table.deleted=0 
        AND $contract_items_table.contract_id=$contract_id 
        AND $contracts_table.deleted=0";

        $total_yearly = $this->db->query($yearly_sql)->getRow();
        ...
        $result->contract_single = $total_single->sum_total_single;
        $result->contract_monthly = $total_monthly->sum_total_monthly;
        $result->contract_quarterly = $total_quarterly->sum_total_quarterly;
        $result->contract_yearly = $total_yearly->sum_total_yearly;
