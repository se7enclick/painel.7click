...
class Items_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $recurrence = get_array_value($options, "recurrence");
        if ($recurrence) {
            $where .= " AND $items_table.recurrence=1";
        }
        ...
        $module_id = get_array_value($options, "module_id");
        if ($module_id) {
            $where .= " AND $items_table.module_id=$module_id";
        }
