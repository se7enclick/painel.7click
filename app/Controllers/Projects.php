...
class Projects extends Security_Controller {
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $project_labels = make_labels_view_data($data->labels_list, false);
        $title =  anchor(get_uri("projects/view/" . $data->id),$project_labels);
  
        $optoins = "";

        if ($this->can_edit_projects()) {
            $optoins .= "<div class='float-start'>" . anchor(get_uri("projects/view/" . $data->id), "<i data-feather='external-link' class='icon-16'></i> ")  . "</div>";
            //$optoins .= modal_anchor(get_uri("contract/preview/"), "<i data-feather='external-link' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_project'), "data-post-id" => $data->contract_id));
        }
        ...
        $row_data = array(
            ...
            // $price,
            anchor(get_uri("contracts/preview/" . $data->contract_id), get_contract_id($data->contract_id)),
            $data->start_date,
            $start_date,
            $data->deadline,
            $dateline,
