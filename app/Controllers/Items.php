...
class Items extends Security_Controller {
    ...
    function index() {
        ...
        $view_data['modules_dropdown'] = $this->_get_modules_dropdown();
    ...
    //get categories dropdown
    private function _get_modules_dropdown() {
        $modules = $this->Item_modules_model->get_all_where(array("deleted" => 0), 0, 0, "title")->getResult();

        $modules_dropdown = array(array("id" => "", "text" => "- " . app_lang("modules") . " -"));
        foreach ($modules as $module) {
            $modules_dropdown[] = array("id" => $module->id, "text" => $module->title);
        }

        return json_encode($modules_dropdown);
    }
    ...
    function modal_form() {
        ...
        $view_data['modules_dropdown'] = $this->Item_modules_model->get_dropdown_list(array("title"));
    ...
    function save() {
        ...
        $item_data = array(
            ...
            "contract_description" => $this->request->getPost('contract_description'),
            "contract_content" => $this->request->getPost('contract_content'),
            "contract_delivery_of_service" => $this->request->getPost('contract_delivery_of_service'),
            
            "category_id" => $this->request->getPost('category_id'),
            "module_id" => $this->request->getPost('category_id'),
            "unit_type" => $this->request->getPost('unit_type'),
            "rate" => unformat_currency($this->request->getPost('item_rate')),
            "recurrence" => $this->request->getPost('recurrence') ? $this->request->getPost('recurrence') : "",
            "item_recurrence_mount" => unformat_currency($this->request->getPost('item_recurrence_mount')),
            "item_recurrence_quarterly" => unformat_currency($this->request->getPost('item_recurrence_quarterly')),
            "item_recurrence_semiannual" => unformat_currency($this->request->getPost('item_recurrence_semiannual')),
            "item_recurrence_yearly" => unformat_currency($this->request->getPost('item_recurrence_yearly')),
